...
import {
  setAssignments,
  removeAssignment as deleteAssignmentRedux,
} from "./reducer";
import {
  fetchAssignments,
...
export default function Assignments() {
  ...
  // Fetch assignments from the server when the component loads or the course ID changes
  useEffect(() => {
    if (cid) {
      fetchAssignments(cid).then((data) => {
  ...
  const confirmDelete = () => {
    if (assignmentToDelete) {
      // Delete assignment on the server
      deleteAssignmentAPI(assignmentToDelete).then(() => {
        // Remove from Redux state after successful deletion
        dispatch(deleteAssignmentRedux(assignmentToDelete));
      });
    }
    setShowDeleteDialog(false);
    setAssignmentToDelete(null);
    ...
    <div id="wd-assignments" className="wd-assignments-container">
      <div className="wd-header">
        <input
          ...
          className="wd-search-input"
          onChange={(e) => {
            // You can add a filter-by-title API integration here if necessary
          }}
        />
        <FaSearch className="wd-search-icon" />
        <div className="wd-buttons">
          <button id="wd-add-assignment-group" className="wd-group-btn">
            + Group
          </button>
          <button
            id="wd-add-assignment"
